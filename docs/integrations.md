...
* **A `STRING_VECTOR` splits on commas and trims nothing**, so `radio-m3u:a.m3u, b.m3u` names a
  second playlist whose path starts with a space. An empty element is dropped.
* **A `BOOL` is `true` and nothing else**, and an empty value is the same as no line at all:
  `fullscreen:True` is false and `mqtt-user:` leaves the compiled-in default in place, neither
  saying a word. `FLAG` has a hardwired `false` default no config line can turn on — which is
  why `fullscreen`, needing to default on, is a `BOOL` and `gate-control` is not.
...
* **A missing config file is not an error** — the module writes a default one with a bare
  `ofstream`: no parent directory is made, and a write that fails says nothing.
  `loadSettings()` creates `~/.config/kuchnia` first and refuses to start when it cannot, then
  sets `0600` on a file that appeared across the call, which the umask would not. It reads
  `--configpath` for itself too, to know whether that per-user path is in use; the module
  parses the same flag again and only its copy picks the file — [packaging](docs/packaging.md).
